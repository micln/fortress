...
- `prototype_main_game.gd` 会按当前运行环境使用不同地图尺度参数：移动端（触屏）默认把地图世界拉得更开，桌面端默认更紧凑；同时开局镜头会轻微偏向玩家出生城，把主城尽量落在顶部 HUD 下方的安全区里。
...
- `scripts/domain/prototype_city_state.gd`
  - 城市状态、防御、产能、人口上限、产兵判断、升级成本与升级应用；当前产能升级步长为 `+0.4`，上限为 `3.0/秒`
...
6. 每秒按各城市产能推进占领城市产兵，并在源城新增士兵时按“每产 1 兵调度 1 次”触发持续任务
7. 每轮推进后通过 `get_winner()` 判断是否只剩一个非中立势力
8. 玩家点 `A -> B` 或 AI 决策出一条 `A -> B` 时，会把路线注册到 `PrototypeOrderDispatchService`
9. 当源城新增士兵时，主循环会立刻按当前目标归属动态选择进攻或运兵并自动派 1 人；这里的新增既包括本地产兵，也包括友军运兵抵达
10. 若同一源城持有多条路线，调度器会按轮转顺序逐条触发
11. 若源城失守，主场景会清空该源城发出的全部持续任务；目标换手不会删除路线，只会改变本次派兵类型
12. 友军运兵到达同阵营城市时，`prototype_transfer_arrival_service.gd` 会按“逐兵到达 -> 逐兵触发调度”的顺序处理；若城市已满员，则把该名到达士兵先当作一次临时可转运兵力，只有调度失败时才记为溢出
