...
当前原型运行在 `main.*` 主场景链路 + `scripts/` 可复用模块这一套实现上：

- 主场景：`scenes/main/main.tscn`
- 主控制器入口壳：`scenes/main/main.gd`（场景仍引用该脚本）
- 城市视图：`scripts/presentation/city_view.gd`
...
- `scenes/main/main.gd`
  - 仅负责 `extends "res://scenes/main/main_bootstrap.gd"`，保持场景引用路径稳定
- `scenes/main/main_bootstrap.gd`
  - 主要负责 `_ready()` 装配：Camera/GameState/March/Input 等模块的 setup 与接线
- `scenes/main/main_flow.gd`
  - 主流程与 UI 编排：开局/暂停/结束、HUD 刷新、按钮回调、对局推进等
- `scenes/main/main_context.gd`
  - 共享基础：常量、节点引用、工具函数、少量供模块注入的 getter/setter
- `main_input_handler.gd`
...
- `preset_map_definition.gd`
  - 第一张中国风预设地图的 design canvas、城市、道路与按方数划分的出生配置
- `preset_map_loader.gd`
  - 校验预设地图模板、映射设计坐标到运行时世界坐标，并生成 `CityState` 数组
- `battle_service.gd`
...
- `scenes/main/main.gd` + `scenes/main/main_*.gd`
...
- 每一步改动都必须维持当前唯一运行链：`scenes/main/main.tscn` 仍引用 `scenes/main/main.gd`。
...
- 运行：在 Godot 中启动 `scenes/main/main.tscn`
